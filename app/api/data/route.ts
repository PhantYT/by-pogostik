import { apiJson, apiOptions, currentProfile, ensureDatabase, getD1, STUDY_GROUPS, validGroupId } from "@/lib/server-data";
...
export async function GET(request: Request) {
  ...
  const db = getD1();
  const [homeworkResult, eventsResult] = await Promise.all([
